...
class ImportCostCenters
{
    ...
    public function index(string $action = ''): void
    {
        ...
        if (!empty($_FILES['file']) && isset($this->data['form']['csrf_token']) && CSRFHelper::validateCSRFToken('form_import_cost_centers', $this->data['form']['csrf_token'])) {
            $this->processFile();
            return;
        }
    ...
    {
        $pageElements = [
            'title_head' => 'Importar Centros de Custo',
        ...
        $loadView = new LoadViewService('adms/Views/costCenter/importCostCenters', $this->data);
        $loadView->loadView();
    }
    ...
    private function processCsv(string $tmpPath): bool
    {
        ...
        $repo = new CostCentersRepository();
            ...
            try {
                ...
                if ($existing) {
                    if (trim($existing['name']) === $name) {
                        ...
                        $ok = $repo->updateCostCenter(['id'=>(int)$existing['id'],'name'=>$name]);
                    ...
                    $okId = $repo->createCostCenter(['name'=>$name]);
                ...
                GenerateLog::generateLog('error','Falha ao importar centro de custo.', ['name'=>$name, 'e'=>$e->getMessage()]);
            }
        }
        fclose($fp);
    ...
    {
        $filename = 'template_importacao_centros_custo.csv';
