 ...
 */
class ValidationPositionService
{
    /**
    ...
    public function validate(array $data): array
    {
        ...
        if (!isset($data['id'])) {
            $rules['name'] = 'required|uniqueInColumns:adms_positions,name';
        } else {
            // Para edição, adicionar validação de id e ignorar o próprio cargo
            $rules['id'] = 'required|integer';
            $rules['name'] = 'required|uniqueInColumns:adms_positions,name,' . $data['id'];
        }
        ...
        $messages = [
            ...
            'email:uniqueInColumns' => 'Já existe um cargo cadastrado com este nome.',
        ];
