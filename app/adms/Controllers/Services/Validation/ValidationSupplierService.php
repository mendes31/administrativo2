 ...
 * 
 * Esta classe é responsável por validar os dados de um formulário de Fornecedor, aplicando regras de validação para criação e edição de Fornecedor.
...
class ValidationSupplierService
{
     ...
     * 
     * Este método valida os dados fornecidos no formulário de fornecedor, aplicando diferentes regras dependendo se é uma criação ou edição de Fornecedor.
     * 
    ...
    public function validate(array $data): array
    {
        ...
        if (!isset($data['id'])) {
            $rules['card_code'] = 'required|uniqueInColumns:adms_supplier,card_code';
            $rules['card_name'] = 'required|uniqueInColumns:adms_supplier,card_name';
        } else {
            // Para edição, adicionar validação de id e ignorar o próprio fornecedor
            $rules['id'] = 'required|integer';
            $rules['card_code'] = 'required|uniqueInColumns:adms_supplier,card_code,' . $data['id'];
            $rules['card_name'] = 'required|uniqueInColumns:adms_supplier,card_name,' . $data['id'];
        }
        ...
        $messages = [
            ...
            'card_name:uniqueInColumns' => 'Já existe um Fornecedor cadastrado com este nome.',
        ];
