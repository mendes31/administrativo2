...
class PaymentsRepository extends DbConnection
{
    ...
    public function updatePay(array $data): bool
    {
        ...
        $stmt = $this->getConnection()->prepare($sql);
        $stmt->bindValue(':num_nota', $data['num_nota']);
        $stmt->bindValue(':card_code_fornecedor', $data['card_code_fornecedor']);
        $stmt->bindValue(':description', $data['description']);
        $stmt->bindValue(':original_value', $this->normalizarValor($data['original_value']));
        $stmt->bindValue(':due_date', $data['due_date']);
        $stmt->bindValue(':expected_date', $data['expected_date']);
        $stmt->bindValue(':partner_id', $data['partner_id']);
        $stmt->bindValue(':frequency_id', $data['frequency_id']);
        $stmt->bindValue(':cost_center_id', $data['cost_center_id']);
        $stmt->bindValue(':account_id', $data['account_id']);
        $stmt->bindValue(':installment_number', $data['installment_number']);
