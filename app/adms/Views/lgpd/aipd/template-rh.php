...
<div class="container-fluid px-4">
    ...
    <div class="mb-1 hstack gap-2">
        ...
        <ol class="breadcrumb mb-3 mt-3 ms-auto">
            ...
            <li class="breadcrumb-item">Template RH</li>
        </ol>
    </div>

    <div class="alert alert-info">
        <h6><i class="fas fa-info-circle"></i> Template Específico para Setor de RH</h6>
        <p class="mb-0">
            <strong>Use este template para atividades que envolvem dados de funcionários, 
            recrutamento, folha de pagamento, benefícios, etc.</strong><br>
            <strong>Exemplos práticos:</strong> Recrutamento e seleção, folha de pagamento, 
            benefícios, avaliações de desempenho, treinamentos, gestão de carreira.
        </p>
    </div>
    ...
    <div class="alert alert-warning">
        ...
        <p class="mb-0">
            <strong>A AIPD deve estar vinculada a uma ROPA específica.</strong> A lista abaixo mostra ROPAs relevantes para o setor de RH. 
    ...
    <div class="card mb-4 border-primary">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0">
                ...
                Avaliação de Impacto à Proteção de Dados - RH
            </h5>
        </div>
        ...
        <div class="card-body">
            ...
            <form action="" method="POST" class="row g-3">
                <input type="hidden" name="csrf_token" value="<?php echo CSRFHelper::generateCSRFToken('form_template_rh'); ?>">

                <!-- Campos Específicos do Setor de RH -->
                <div class="col-12">
                    <div class="card border-primary">
                        <div class="card-header bg-primary text-white">
                            <h6 class="mb-0">Informações Específicas do Setor de RH</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <label for="tipo_processo" class="form-label">
                                        <?php echo $this->data['template_rh']['campos_especificos']['tipo_processo']['label']; ?>
                                    </label>
                                    <select name="tipo_processo" class="form-select" id="tipo_processo">
                                        <option value="">Selecione</option>
                                        <?php foreach ($this->data['template_rh']['campos_especificos']['tipo_processo']['options'] as $valor => $texto): ?>
                                            <option value="<?php echo $valor; ?>" 
                                                    <?php echo (isset($this->data['form']['tipo_processo']) && $this->data['form']['tipo_processo'] === $valor) ? 'selected' : ''; ?>>
                                ...
                                <div class="col-md-6">
                                    <label for="categoria_funcionarios" class="form-label">
                                        <?php echo $this->data['template_rh']['campos_especificos']['categoria_funcionarios']['label']; ?>
                                    </label>
                                    <select name="categoria_funcionarios" class="form-select" id="categoria_funcionarios">
                                        <option value="">Selecione</option>
                                        <?php foreach ($this->data['template_rh']['campos_especificos']['categoria_funcionarios']['options'] as $valor => $texto): ?>
                                            <option value="<?php echo $valor; ?>" 
                                                    <?php echo (isset($this->data['form']['categoria_funcionarios']) && $this->data['form']['categoria_funcionarios'] === $valor) ? 'selected' : ''; ?>>
                                                <?php echo htmlspecialchars($texto); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <div class="row mt-3">
                                <div class="col-12">
                                    <label class="form-label">
                                        <?php echo $this->data['template_rh']['campos_especificos']['dados_tratados']['label']; ?>
                                    </label>
                                    <div class="row">
                                        <?php foreach ($this->data['template_rh']['campos_especificos']['dados_tratados']['options'] as $valor => $texto): ?>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6">
                                    <label for="base_legal" class="form-label">
                                        <?php echo $this->data['template_rh']['campos_especificos']['base_legal']['label']; ?>
                                    </label>
                                    <select name="base_legal" class="form-select" id="base_legal">
                                        <option value="">Selecione</option>
                                        <?php foreach ($this->data['template_rh']['campos_especificos']['base_legal']['options'] as $valor => $texto): ?>
                                            <option value="<?php echo $valor; ?>" 
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <input type="text" name="titulo" class="form-control" id="titulo" 
                                           value="<?php echo $this->data['form']['titulo'] ?? $this->data['template_rh']['titulo_padrao']; ?>" required>
                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (nivelRiscoSelect && !nivelRiscoSelect.value) {
        nivelRiscoSelect.value = 'Alto'; // Padrão para RH
    }
});
</script>
