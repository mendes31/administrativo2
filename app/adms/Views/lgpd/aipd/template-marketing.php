...
<div class="container-fluid px-4">
    ...
    <div class="mb-1 hstack gap-2">
        ...
        <ol class="breadcrumb mb-3 mt-3 ms-auto">
            ...
            <li class="breadcrumb-item">Template Marketing</li>
        </ol>
    </div>

    <div class="alert alert-info">
        <h6><i class="fas fa-info-circle"></i> Template Específico para Setor de Marketing</h6>
        <p class="mb-0">
            <strong>Use este template para atividades que envolvem dados de clientes, 
            campanhas publicitárias, análise de comportamento, etc.</strong><br>
            <strong>Exemplos práticos:</strong> Campanhas de marketing, análise de clientes, 
            publicidade direcionada, CRM, redes sociais, email marketing, analytics.
        </p>
    </div>
    ...
    <div class="alert alert-warning">
        ...
        <p class="mb-0">
            <strong>A AIPD deve estar vinculada a uma ROPA específica.</strong> A lista abaixo mostra ROPAs relevantes para o setor de marketing. 
    ...
    <div class="card mb-4 border-warning">
        <div class="card-header bg-warning text-dark">
            <h5 class="mb-0">
                ...
                Avaliação de Impacto à Proteção de Dados - Marketing
            </h5>
        </div>
        ...
        <div class="card-body">
            ...
            <form action="" method="POST" class="row g-3">
                <input type="hidden" name="csrf_token" value="<?php echo CSRFHelper::generateCSRFToken('form_template_marketing'); ?>">

                <!-- Campos Específicos do Setor de Marketing -->
                <div class="col-12">
                    <div class="card border-warning">
                        <div class="card-header bg-warning text-dark">
                            <h6 class="mb-0">Informações Específicas do Setor de Marketing</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <label for="tipo_campanha" class="form-label">
                                        <?php echo $this->data['template_marketing']['campos_especificos']['tipo_campanha']['label']; ?>
                                    </label>
                                    <select name="tipo_campanha" class="form-select" id="tipo_campanha">
                                        <option value="">Selecione</option>
                                        <?php foreach ($this->data['template_marketing']['campos_especificos']['tipo_campanha']['options'] as $valor => $texto): ?>
                                            <option value="<?php echo $valor; ?>" 
                                                    <?php echo (isset($this->data['form']['tipo_campanha']) && $this->data['form']['tipo_campanha'] === $valor) ? 'selected' : ''; ?>>
                                ...
                                <div class="col-md-6">
                                    <label for="canal_marketing" class="form-label">
                                        <?php echo $this->data['template_marketing']['campos_especificos']['canal_marketing']['label']; ?>
                                    </label>
                                    <select name="canal_marketing" class="form-select" id="canal_marketing">
                                        <option value="">Selecione</option>
                                        <?php foreach ($this->data['template_marketing']['campos_especificos']['canal_marketing']['options'] as $valor => $texto): ?>
                                            <option value="<?php echo $valor; ?>" 
                                                    <?php echo (isset($this->data['form']['canal_marketing']) && $this->data['form']['canal_marketing'] === $valor) ? 'selected' : ''; ?>>
                                                <?php echo htmlspecialchars($texto); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <div class="row mt-3">
                                <div class="col-12">
                                    <label class="form-label">
                                        <?php echo $this->data['template_marketing']['campos_especificos']['dados_tratados']['label']; ?>
                                    </label>
                                    <div class="row">
                                        <?php foreach ($this->data['template_marketing']['campos_especificos']['dados_tratados']['options'] as $valor => $texto): ?>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6">
                                    <label for="base_legal" class="form-label">
                                        <?php echo $this->data['template_marketing']['campos_especificos']['base_legal']['label']; ?>
                                    </label>
                                    <select name="base_legal" class="form-select" id="base_legal">
                                        <option value="">Selecione</option>
                                        <?php foreach ($this->data['template_marketing']['campos_especificos']['base_legal']['options'] as $valor => $texto): ?>
                                            <option value="<?php echo $valor; ?>" 
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <input type="text" name="titulo" class="form-control" id="titulo" 
                                           value="<?php echo $this->data['form']['titulo'] ?? $this->data['template_marketing']['titulo_padrao']; ?>" required>
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="objetivo" class="form-control" id="objetivo" rows="3" required><?php echo $this->data['form']['objetivo'] ?? $this->data['template_marketing']['objetivo_padrao']; ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="escopo" class="form-label">Escopo</label>
                                    <textarea name="escopo" class="form-control" id="escopo" rows="3"><?php echo $this->data['form']['escopo'] ?? $this->data['template_marketing']['escopo_padrao']; ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="metodologia" class="form-label">Metodologia</label>
                                    <textarea name="metodologia" class="form-control" id="metodologia" rows="3"><?php echo $this->data['form']['metodologia'] ?? $this->data['template_marketing']['metodologia_padrao']; ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="riscos_identificados" class="form-label">Riscos Identificados</label>
                                    <textarea name="riscos_identificados" class="form-control" id="riscos_identificados" rows="4"><?php echo $this->data['form']['riscos_identificados'] ?? implode("\n", $this->data['template_marketing']['riscos_identificados']); ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="medidas_mitigacao" class="form-label">Medidas de Mitigação</label>
                                    <textarea name="medidas_mitigacao" class="form-control" id="medidas_mitigacao" rows="4"><?php echo $this->data['form']['medidas_mitigacao'] ?? implode("\n", $this->data['template_marketing']['medidas_mitigacao']); ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="conclusoes" class="form-label">Conclusões</label>
                                    <textarea name="conclusoes" class="form-control" id="conclusoes" rows="3"><?php echo $this->data['form']['conclusoes'] ?? $this->data['template_marketing']['conclusoes_padrao']; ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="recomendacoes" class="form-label">Recomendações</label>
                                    <textarea name="recomendacoes" class="form-control" id="recomendacoes" rows="4"><?php echo $this->data['form']['recomendacoes'] ?? implode("\n", $this->data['template_marketing']['recomendacoes_padrao']); ?></textarea>
                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (nivelRiscoSelect && !nivelRiscoSelect.value) {
        nivelRiscoSelect.value = 'Médio'; // Padrão para marketing
    }
});
</script>
