...
<div class="container-fluid px-4">
    ...
    <div class="mb-1 hstack gap-2">
        ...
        <ol class="breadcrumb mb-3 mt-3 ms-auto">
            ...
            <li class="breadcrumb-item">Template Educação</li>
        </ol>
    </div>

    <div class="alert alert-info">
        <h6><i class="fas fa-info-circle"></i> Template Específico para Setor de Educação</h6>
        <p class="mb-0">
            <strong>Use este template para atividades que envolvem dados educacionais, acadêmicos, 
            treinamentos, cursos, etc.</strong><br>
            <strong>Exemplos práticos:</strong> Gestão de alunos, matrículas, notas, frequência, 
            treinamentos corporativos, cursos online, capacitações, avaliações de desempenho.
        </p>
    </div>
    ...
    <div class="alert alert-warning">
        ...
        <p class="mb-0">
            <strong>A AIPD deve estar vinculada a uma ROPA específica.</strong> A lista abaixo mostra ROPAs relevantes para o setor de educação. 
    ...
    <div class="card mb-4 border-info">
        <div class="card-header bg-info text-white">
            <h5 class="mb-0">
                ...
                Avaliação de Impacto à Proteção de Dados - Setor de Educação
            </h5>
        </div>
        ...
        <div class="card-body">
            ...
            <form action="" method="POST" class="row g-3">
                <input type="hidden" name="csrf_token" value="<?php echo CSRFHelper::generateCSRFToken('form_template_educacao'); ?>">

                <!-- Campos Específicos do Setor de Educação -->
                <div class="col-12">
                    <div class="card border-info">
                        <div class="card-header bg-info text-white">
                            <h6 class="mb-0">Informações Específicas do Setor de Educação</h6>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <label for="tipo_instituicao" class="form-label">
                                        <?php echo $this->data['template_educacao']['campos_especificos']['tipo_instituicao']['label']; ?>
                                    </label>
                                    <select name="tipo_instituicao" class="form-select" id="tipo_instituicao">
                                        <option value="">Selecione</option>
                                        <?php foreach ($this->data['template_educacao']['campos_especificos']['tipo_instituicao']['options'] as $valor => $texto): ?>
                                ...
                                <div class="col-md-6">
                                    <label for="nivel_ensino" class="form-label">
                                        <?php echo $this->data['template_educacao']['campos_especificos']['nivel_ensino']['label']; ?>
                                    </label>
                                    <select name="nivel_ensino" class="form-select" id="nivel_ensino">
                                        <option value="">Selecione</option>
                                        <?php foreach ($this->data['template_educacao']['campos_especificos']['nivel_ensino']['options'] as $valor => $texto): ?>
                                            <option value="<?php echo $valor; ?>" 
                                                    <?php echo (isset($this->data['form']['nivel_ensino']) && $this->data['form']['nivel_ensino'] === $valor) ? 'selected' : ''; ?>>
                                                <?php echo htmlspecialchars($texto); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <div class="row mt-3">
                                <div class="col-12">
                                    <label class="form-label">
                                        <?php echo $this->data['template_educacao']['campos_especificos']['dados_tratados']['label']; ?>
                                    </label>
                                    <div class="row">
                                        <?php foreach ($this->data['template_educacao']['campos_especificos']['dados_tratados']['options'] as $valor => $texto): ?>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6">
                                    <label for="base_legal" class="form-label">
                                        <?php echo $this->data['template_educacao']['campos_especificos']['base_legal']['label']; ?>
                                    </label>
                                    <select name="base_legal" class="form-select" id="base_legal">
                                        <option value="">Selecione</option>
                                        <?php foreach ($this->data['template_educacao']['campos_especificos']['base_legal']['options'] as $valor => $texto): ?>
                                            <option value="<?php echo $valor; ?>" 
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <input type="text" name="titulo" class="form-control" id="titulo" 
                                           value="<?php echo $this->data['form']['titulo'] ?? $this->data['template_educacao']['titulo_padrao']; ?>" required>
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="objetivo" class="form-control" id="objetivo" rows="3" required><?php echo $this->data['form']['objetivo'] ?? $this->data['template_educacao']['objetivo_padrao']; ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="escopo" class="form-label">Escopo</label>
                                    <textarea name="escopo" class="form-control" id="escopo" rows="3"><?php echo $this->data['form']['escopo'] ?? $this->data['template_educacao']['escopo_padrao']; ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="metodologia" class="form-label">Metodologia</label>
                                    <textarea name="metodologia" class="form-control" id="metodologia" rows="3"><?php echo $this->data['form']['metodologia'] ?? $this->data['template_educacao']['metodologia_padrao']; ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="riscos_identificados" class="form-label">Riscos Identificados</label>
                                    <textarea name="riscos_identificados" class="form-control" id="riscos_identificados" rows="4"><?php echo $this->data['form']['riscos_identificados'] ?? implode("\n", $this->data['template_educacao']['riscos_identificados']); ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="medidas_mitigacao" class="form-label">Medidas de Mitigação</label>
                                    <textarea name="medidas_mitigacao" class="form-control" id="medidas_mitigacao" rows="4"><?php echo $this->data['form']['medidas_mitigacao'] ?? implode("\n", $this->data['template_educacao']['medidas_mitigacao']); ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="conclusoes" class="form-label">Conclusões</label>
                                    <textarea name="conclusoes" class="form-control" id="conclusoes" rows="3"><?php echo $this->data['form']['conclusoes'] ?? $this->data['template_educacao']['conclusoes_padrao']; ?></textarea>
                                </div>

                                <div class="col-12">
                                    <label for="recomendacoes" class="form-label">Recomendações</label>
                                    <textarea name="recomendacoes" class="form-control" id="recomendacoes" rows="4"><?php echo $this->data['form']['recomendacoes'] ?? implode("\n", $this->data['template_educacao']['recomendacoes_padrao']); ?></textarea>
                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (nivelRiscoSelect && !nivelRiscoSelect.value) {
        nivelRiscoSelect.value = 'Alto'; // Padrão para educação
    }
});
</script>
