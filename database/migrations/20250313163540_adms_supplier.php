...
final class AdmsSupplier extends AbstractMigration
{
    /**
     * Cria a tabela AdmsSupplier.
     *
     * Este método é executado durante a aplicação da migração para criar a tabela `adms_supplier` no banco de dados.
    ...
    public function up(): void
    {
        ...
        // Verifica se a tabela 'adms_supplier' não existe no banco de dados
        if (!$this->hasTable('adms_supplier')) {
            // Cria a tabela 'adms_supplier'
            $table = $this->table('adms_supplier');
    ...
    /**
     * Reverte a criação da tabela adms_supplier.
     *
     * Este método é executado durante a reversão da migração para remover a tabela `adms_supplier` do banco de dados.
     * 
     * @return void
     */
    public function down(): void
    {
        // Remove a tabela 'adms_supplier' do banco de dados
        $this->table('adms_supplier')->drop()->save();
    }
}