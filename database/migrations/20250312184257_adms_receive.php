...
final class AdmsReceive extends AbstractMigration
{
    ...
    public function up(): void
    {
        ...
        // Verifica se a tabela 'adms_receive' não existe no banco de dados
        if (!$this->hasTable('adms_receive')) {
            // Cria a tabela 'adms_receive'
    ...
    /**
     * Reverte a criação da tabela adms_receive.
     *
     * Este método é executado durante a reversão da migração para remover a tabela `adms_receive` do banco de dados.
     * 
     * @return void
     */
    public function down(): void
    {
        // Remove a tabela 'adms_receive' do banco de dados
        $this->table('adms_receive')->drop()->save();
    }
}